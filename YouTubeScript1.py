...
def read_creds(config):
    parser = cfg.ConfigParser()
    parser.read(config)
    return parser.get('creds', 'email_id'), parser.get('creds','email_pass')
...
def gmailLogin():
    ...
    smtpserver.ehlo()
    # Authentication
    id,passw = read_creds("config.cfg")
    smtpserver.login(id,passw)


def sendPlainEmail(unUploadedSongName,unUploadedSongCount,UploadedSongCount,UploadedSongName):
    gmailLogin()
    message = "Today Youtube data uploaded Details \n\n1. Total Uploaded Song Count = " + str(UploadedSongCount) + '\n 2. Total Uploaded song Channel Names = '+ str(UploadedSongName) + "\n 3. Total unUploaded Song Count = "+ str(unUploadedSongCount)+ '\n 4. Total unUploaded song Channel Names = '+str(unUploadedSongName)
    smtpserver.sendmail("Ghuguti2019@gmail.com", "technowebtech@gmail.com", message)
...
while True:
    ...
    today_11_45_PM = current_time.replace(hour=20, minute =18, second=0)
    if current_time == today_11_45_PM:
        ...
        for channel in  range(len(channel_data)):
                ...
                if today_date==channel_date:
                        ...
                        resp = response("https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=5&order=date&q=site%3Ayoutube.com&channelId="+channel_id+"&key="+authKey)
